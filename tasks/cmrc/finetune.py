...
def main(cfg: DictConfig) -> None:
    ...
    if not cfg.pretrained_model:
        ...
        if os.path.exists(cfg.pretrained_model):
            logging.info(f'Restore from NeMo model: {cfg.pretrained_model}')
            model = QAModel.restore_from(cfg.pretrained_model)
        elif cfg.pretrained_model in QAModel.get_available_model_names():
            model = QAModel.from_pretrained(cfg.pretrained_model)
        else:
            raise ValueError(
                f'Provide path to the pre-trained .nemo file or choose from '
                f'{QAModel.list_available_models()}'
            )
