...
class FusedDispatch(torch.autograd.Function):
    ...
    def forward(ctx, x, token_indices, token_probs, num_experts, group, previous_event=None):
        ...
        recv_x = to_float8_tensor(recv_x[0], recv_x[1])

        logger.debug(
            f"[FP8 All2All] After DeepEP dispatch. Recv qx: {list(get_data_from_float8(recv_x).shape)}, sx: {list(get_scale_from_float8(recv_x).shape)}"
...
class FusedCombine(torch.autograd.Function):
    ...
    def backward(ctx, grad_output, previous_event=None):
        ...
        grad_x = to_float8_tensor(grad_x[0], grad_x[1])
