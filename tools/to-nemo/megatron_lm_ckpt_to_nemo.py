...
def update_config(config_path,
                  vocab_filename,
                  out_yaml):
  print("Update config from file: {}".format(config_path))
  config = OmegaConf.load(config_path)
  
  config.model.megatron_legacy = False # set to true if you trained the NLP model on NeMo < 1.5.0
  config.model.bias_gelu_fusion = True # set to true if you want the MegatronLM to NeMo conversion for training; and set to false to use the converted model at time of export
  config.model.masked_softmax_fusion = True # set to true if you want the MegatronLM to NeMo conversion for training; and set to false to use the converted model at time of export
  
  config.model.max_position_embeddings = 128
  config.model.data.seq_length = 128
  config.model.num_layers = 12
  config.model.hidden_size = 768
  config.model.ffn_hidden_size = 768 * 4
  ...
  config.model.tokenizer.special_tokens = ''
  config.model.tokenizer.tokenizer_model = None
  config.model.tensor_model_parallel_size = 1
  config.model.data.data_prefix = ''
  config.cfg = {}
  config.cfg.cfg = config.model

  out_yaml = 'hparams.yaml'
  print("Save config to file: {}".format(out_file))
...
if __name__ == '__main__':
   ...
   #config_path = "./megatron_bert_config.yaml"
   #vocab_filename = "/home/jqi/work/megatron/vocab/jq/jq-tokens.txt.2.vocab"
   #update_config(config_path, args.vocab_filename, args.hparams_file)
